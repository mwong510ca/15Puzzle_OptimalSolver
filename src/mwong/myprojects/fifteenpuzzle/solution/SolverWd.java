...
/**
 * SolverWd extends SolverSetup. It is the 15 puzzle optimal solver.
 ...
 */
final class SolverWd extends SolverSetup {
  /**
   * Initializes SolverWd object.
   *
   * @param appMode the applicationMode
   */
  SolverWd(final ApplicationMode appMode) {
    super(HeuristicOptions.WD);
  ...
  void setPriorityBasis(final Board board) throws IllegalArgumentException {
    ...
    transWdCombo();
    priorityBasis = initWdCombo[WD_KEY_ORDER_H_VAL] + initWdCombo[WD_KEY_ORDER_V_VAL];
  }

  @Override
  void dfsStartingOrder(final int limit) {
    dfsStartingOrder(zeroX, zeroY, limit, initWdCombo[WD_KEY_ORDER_H_IDX],
        ...
        switch (Board.Move.values()[firstMoveIdx]) {
          case RIGHT:
            lastDepthSummary[firstMoveIdx] = shiftRight(orgX, orgY, zeroPos, zeroMirror,
                1, limit, idxH, idxV, valH, valV, RESET_VAL);
            break;
          case DOWN:
  ...
  private int dfsNext(final int orgX, final int orgY, final int zeroPos, final int zeroMirror,
      final int idxH, final int idxV, final int valH, final int valV, final int cost,
      final int limit, final int estimate, final int chain, final Board.Move currMove) {
    idaCount++;
    if (terminated) {
  ...
  private int shiftRight(final int orgX, final int orgY, final int zeroPos, final int zeroMirror,
      final int cost, final int limit, final int idxH, final int idxV, final int valH,
      final int valV, final int chain) {
    if (terminated) {
      return END_OF_SEARCH;
    }
  ...
  private int shiftDown(final int orgX, final int orgY, final int zeroPos, final int zeroMirror,
    ...
    if (priority < limit) {
      ...
      swap(zeroPos, nextPos, zeroMirror, nextMirror);
      priority = dfsNext(orgX, orgY + 1, nextPos, nextMirror, shiftIdx, idxV,
          shiftVal, valV, cost + 1, limit - 1, priority, chain, MOVE_DN);
      swap(nextPos, zeroPos, nextMirror, zeroMirror);
    }
    return priority;
  }
  ...
  private int shiftLeft(final int orgX, final int orgY, final int zeroPos, final int zeroMirror,
    ...
    if (priority < limit) {
      ...
      int nextMirror = zeroMirror - ROW_SIZE;
      swap(zeroPos, nextPos, zeroMirror, nextMirror);
      priority = dfsNext(orgX - 1, orgY, nextPos, nextMirror, idxH, shiftIdx,
          valH, shiftVal, cost + 1, limit - 1, priority, chain, MOVE_LT);
      swap(nextPos, zeroPos, nextMirror, zeroMirror);
    }
    return priority;
  }
  ...
  private int shiftUp(final int orgX, final int orgY, final int zeroPos, final int zeroMirror,
    ...
    if (priority < limit) {
      ...
      swap(zeroPos, nextPos, zeroMirror, nextMirror);
      priority = dfsNext(orgX, orgY - 1, nextPos, nextMirror, shiftIdx, idxV,
          shiftVal, valV, cost + 1, limit - 1, priority, chain, MOVE_UP);
      swap(nextPos, zeroPos, nextMirror, zeroMirror);
    }
    return priority;
  }
