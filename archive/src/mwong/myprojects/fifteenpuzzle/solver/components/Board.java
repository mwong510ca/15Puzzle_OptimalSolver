...
public class Board implements Serializable {
    ...
    private void generateBoard(PuzzleDifficultyLevel level) {
        if (level == PuzzleDifficultyLevel.MODERATE) {
            ...
            while (true) {
                ...
                tiles = new byte[SIZE];
                System.arraycopy(blocks, 0, tiles, 0, SIZE);
                if (level == PuzzleDifficultyLevel.HARD && heuristic() > 45) {
                    break;
    ...
    // check if given blocks is goal state
    ...
    /**
     * Returns the boolean represent this board is a symmetry board.
     *
     * @return boolean represent this board is a symmetry board
     */
